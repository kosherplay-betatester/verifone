...
class VerifoneApp(QMainWindow):
    """GUI for Verifone P400: register, key exchange, and status with MAC persistence."""
    ...
    def __init__(self):
        ...
        # Load existing MAC key if available
        self._load_mac()
        ...
        if '<COMMAND>EXCHANGE_KEYS' in sent and '<MAC_KEY>' in received:
            ...
            if match:
                try:
                    ...
                    self.status_btn.setEnabled(True)
                    self._save_mac()
                    # Immediately send a STATUS to verify connection
                    self.get_status()
                except Exception as e:
