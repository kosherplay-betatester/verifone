...
# 2025‑05‑07  v1.32‑C
#
# Changelog (since v1.31‑C)
# ──────────────────────────────────────────────────────────────────────────
# • Credit‑term wizard pops up after DISCOVERY when card supports credit /
#   instalments, lets cashier choose term, number of payments, first / next
#   payment amounts.
# • AUTHORIZE now includes <CREDIT_TERMS>, <PAYMENTS_NUMBER>,
#   <FIRST_PAYMENT_AMOUNT>, <NEXT_PAYMENT_AMOUNT> when applicable.
# • Full error checking, minor‑unit conversion, high‑DPI support.
# -------------------------------------------------------------------------
...
class CreditTermDlg(QDialog):
    """Select CREDIT_TERMS / payments / first / next amounts."""
    ...
    def __init__(self, flags: Dict[str, bool],
                 min_pay: int, max_pay: int,
                 total_amount: float,
                 parent=None):
        super().__init__(parent)
        self.total = total_amount
        self.setWindowTitle("Choose credit / installment details")
        ...
        for lbl, code in self._map.items():
            field = {
                '1': 'regular', '2': 'special',
                '3': 'immediate', '6': 'credit', '8': 'installments'
            }[code]
            self.cmb.addItem(lbl, code)
            it = self.cmb.model().item(self.cmb.count() - 1)
            it.setEnabled(flags.get(field, False))
        g.addWidget(self.cmb, r, 0, 1, 2); r += 1

        # payments
        self.lbl_pay, self.spin_pay = QLabel("Payments:"), QSpinBox()
        self.spin_pay.setRange(min_pay, max_pay)
        self.spin_pay.setValue(min_pay)
        g.addWidget(self.lbl_pay,  r, 0); g.addWidget(self.spin_pay,  r, 1); r += 1

        # first / next
        self.lbl_first, self.lbl_next = QLabel("First payment (₪):"), QLabel("Next payments (₪):")
        self.spin_first = QDoubleSpinBox(decimals=2, maximum=self.total, value=self.total / self.spin_pay.value())
        self.spin_next  = QDoubleSpinBox(decimals=2, maximum=self.total, value=self.spin_first.value())
        ...
        self.cmb.currentIndexChanged.connect(self._toggle_widgets)
        self.spin_pay.valueChanged.connect(self._recalc_next)
        self.spin_first.valueChanged.connect(self._recalc_next)
    ...
    def _recalc_next(self, *_):
        if not self.spin_first.isVisible():
            return
        p = self.spin_pay.value()
        first = self.spin_first.value()
        rest = max(0.0, self.total - first)
        next_p = rest / max(1, p - 1)
        self.spin_next.blockSignals(True)
        self.spin_next.setValue(round(next_p, 2))
        self.spin_next.blockSignals(False)

    def data(self) -> Tuple[str, int, float, float]:
        """Return (credit_term, payments, first_amt, next_amt)."""
        code = self.cmb.currentData()
        if code in ('6', '8'):
            return (code,
                    self.spin_pay.value(),
                    self.spin_first.value(),
                    self.spin_next.value())
        return code, 0, 0.0, 0.0


# ───────────────────── other dialogs (identical as before) ────────────────
...
class DiscoverDlg(QDialog):
    """Unchanged from earlier version – kept for brevity."""
    def __init__(self, parent=None):
        super().__init__(parent)
...
class Main(QMainWindow):
    def __init__(self):
        ...
        srow.addWidget(self.search_edit); self.search_btn = QPushButton("Find", clicked=self.search_logs)
        srow.addWidget(self.search_btn); srow.addStretch(); lay.addLayout(srow)
    ...
    def _auth_body(self, defaults: Dict[str, str],
                   cred_term: str, payments: int,
                   first_amt: float, next_amt: float) -> str:
        x = self._disc_body(defaults)
        inject = f"<CREDIT_TERMS>{cred_term}</CREDIT_TERMS>"
        if payments:
            inject += f"<PAYMENTS_NUMBER>{payments:02d}</PAYMENTS_NUMBER>"
        if first_amt:
            inject += f"<FIRST_PAYMENT_AMOUNT>{to_minor(first_amt)}</FIRST_PAYMENT_AMOUNT>"
        if next_amt:
            inject += f"<NEXT_PAYMENT_AMOUNT>{to_minor(next_amt)}</NEXT_PAYMENT_AMOUNT>"
    ...
    def _handle(self, sent_xml: str, recv_xml: str):
        ...
        if self.qs_active:
            ...
            if '<COMMAND>DISCOVERY' in sent_xml:
                ...
                def flag(name): return bool(re.search(fr'<TERMS_{name.upper()}>1</TERMS_{name.upper()}>', recv_xml, re.I))
                flags = {f: flag(f) for f in ('regular', 'special', 'immediate', 'credit', 'installments')}
                credit_ok = flags['credit'] or flags['installments']

                if credit_ok:
                    def int_tag(tag, dflt):
                        m = re.search(fr'<{tag}>(\d+)</{tag}>', recv_xml)
                        return int(m.group(1)) if m else dflt
                    min_pay = max(2, int_tag('CREDIT_MIN_PAYMENTS', 2))
                    max_pay = max(min_pay, int_tag('CREDIT_MAX_PAYMENTS', 36))
                    dlg = CreditTermDlg(flags, min_pay, max_pay, self.qs_amt.value(), self)
                    if dlg.exec_() != QDialog.Accepted:
                        QMessageBox.information(self, "Quick Sale", "Cancelled by user.")
                        self.qs_active = False
                        return
                    cred_term, payments, first_amt, next_amt = dlg.data()
                else:
                    cred_term, payments, first_amt, next_amt = '1', 0, 0.0, 0.0

                auth_body = self._auth_body(self.qs_defaults,
                                            cred_term, payments,
                                            first_amt, next_amt)
