#!/usr/bin/env python3
# Verifone P400 connector – Quick‑Sale + webhook (product description)
# 2025‑05‑07  v1.34
#
# Changelog v1.33 → v1.34
# ──────────────────────────────────────────────────────────────────────────
# • Webhook accepts ?amount=…&product=…          (GET /pay)
# • Signal carries (amount, product)
# • Product embedded in AUTHORIZE/DISCOVERY XML  (<FREE_TEXT>)
# • Receipt pop‑up prefixed with “Product: …”
            ...
            with socket.create_connection((self.ip, self.port), timeout=7) as s:
                ...
                while True:
                    try:
                        ...
                        break
                received = b''.join(buf).decode('utf‑8', 'replace')
...
class WebhookServer(QThread):
    """Listen on /pay?amount=..&product=.. ; emit (amount, product)."""
    receivedSale = pyqtSignal(float, str)
    ...
    def run(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                q = parse_qs(p.query)
                amt_raw = q.get("amount", [])
                if not amt_raw:
                    self._reply(400, b"amount parameter required"); return
                try:
                    amt = float(amt_raw[0])
                    if amt <= 0: raise ValueError
                except ValueError:
                    self._reply(400, b"amount must be positive number"); return

                # optional product
                prod = q.get("product", [""])[0]
                prod = unquote_plus(prod)[:60]  # limit length for safety

                self._reply(200, b"OK")
                parent.receivedSale.emit(amt, prod)
...
class CreditTermDlg(QDialog):
    ...
    def __init__(self, flags: Dict[str,bool], min_p: int, max_p: int,
        ...
        self.spin_pay.setRange(min_p, max_p); self.spin_pay.setValue(min_p)
        g.addWidget(self.lbl_pay, r, 0); g.addWidget(self.spin_pay, r, 1); r += 1

        self.lbl_first = QLabel("First payment (₪):")
        self.spin_first = QDoubleSpinBox(decimals=2, maximum=total,
                                         value=total/self.spin_pay.value())
        self.lbl_next  = QLabel("Next payments (₪):")
        self.spin_next = QDoubleSpinBox(decimals=2, maximum=total,
                                        value=self.spin_first.value()); self.spin_next.setReadOnly(True)
...
# ───────── StartTransactionDlg / DiscoverDlg identical to v1.33 (omitted) ─
...
class Main(QMainWindow):
    def __init__(self):
        ...
        # --- UI build (same as v1.33; only labels modified) --------------
        central = QWidget(); lay = QVBoxLayout(central)

        cr = QHBoxLayout(); self.ip=QLineEdit("192.168.1.202"); self.port=QLineEdit("5015")
        cr.addWidget(QLabel("IP:")); cr.addWidget(self.ip)
        cr.addWidget(QLabel("Port:")); cr.addWidget(self.port); cr.addStretch(); lay.addLayout(cr)
        ...
        kr = QHBoxLayout(); kr.addWidget(QLabel("KTK (16)"))
        ...
        mr = QHBoxLayout(); mr.addWidget(QLabel("MAC Key"))
        self.mac_view=QLineEdit(); self.mac_view.setReadOnly(True); mr.addWidget(self.mac_view); mr.addStretch()
        lay.addLayout(mr)
        ...
        br.addWidget(QLabel("Amount ₪")); self.qs_amt = QDoubleSpinBox(decimals=2, maximum=999999, value=10.00)
        ...
        srow=QHBoxLayout(); srow.addWidget(QLabel("Search logs:")); self.search_edit=QLineEdit()
        ...
        self.webhook.receivedSale.connect(self._from_webhook)
        self.webhook.start()
    ...
    def _disc_body(self,d:Dict[str,Optional[str]])->str:
        ...
        if self.qs_product:
            x += f"<FREE_TEXT>{html.escape(self.qs_product)}</FREE_TEXT>"
    ...
    def _auth_body(self,defl:Dict[str,str], ct:str,pay:int,first:float,nxt:float)->str:
        x=self._disc_body(defl)
    ...
    # ───────── quick‑sale entry ───────────────────────────────────────────
    def quick_sale(self,amount:float,product:str):
        if self.qs_active:
            QMessageBox.warning(self,"Quick Sale","Already running."); return
        self.qs_product = product
    ...
    @pyqtSlot(float,str)
    def _from_webhook(self, amt:float, prod:str):
        self.quick_sale(amt, prod)
    ...
    def _handle(self,sent:str,recv:str):
        ...
        if self.qs_active:
            ...
            if '<COMMAND>DISCOVERY' in sent:
                ...
                cred_ok=flags['credit'] or flags['installments']
                if cred_ok:
                    def it(tag,d): m=re.search(fr'<{tag}>(\d+)</{tag}>',recv); return int(m.group(1)) if m else d
                    min_p=max(2,it('CREDIT_MIN_PAYMENTS',2)); max_p=max(min_p,it('CREDIT_MAX_PAYMENTS',36))
                    dlg=CreditTermDlg(flags,min_p,max_p,self.qs_amt.value(),self)
