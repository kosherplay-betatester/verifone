...
# • Every successful PAYMENT/AUTHORIZE (RESULT_CODE 0) writes a UTF-8 receipt in
#   ./receipts/YYMMDD_HHMMSS_receipt.txt
# • Free-Call sandbox (MX-Tester style) with four presets
# • v1.44 – every Free-Call is re-signed so that <MAC> is inserted immediately
#   after </TRANSACTION_TIME>.  Old MAC blocks are stripped automatically.
...
from PyQt5.QtWidgets import (
    ...
    QTextEdit, QSpinBox, QDoubleSpinBox, QDateEdit, QCheckBox, QGridLayout,
    QVBoxLayout, QHBoxLayout, QSplitter, QMessageBox, QDialog, QDialogButtonBox
)
...
class CreditTermDlg(QDialog):
    _map = {'01 רגילה':'1','02 דחויה(+30)':'2','03 מיידית':'3','06 אשראי':'6','08 תשלומים':'8'}
...
class Main(QMainWindow):
    def __init__(self):
        ...
        con.addWidget(self.training_chk); lay.addLayout(con)
        ...
        split=QSplitter(Qt.Horizontal); split.addWidget(self.sent); split.addWidget(self.recv); lay.addWidget(split,1)
        ...
        # Free-Call panel
        fcbox=QVBoxLayout(); fcbox.addWidget(QLabel("<b>Free Call XML Sandbox</b>"))
        self.free_xml=QTextEdit(); fcbox.addWidget(self.free_xml,1)
        fbtns=QHBoxLayout()
        self.free_send_btn=QPushButton("Send Free Call",clicked=self.free_send); self.free_send_btn.setEnabled(False)
        fbtns.addWidget(self.free_send_btn)
        self.free_cmds=[]
        for fg,cmd,label in (('SESSION','START_TRAN','Start Tran'),
                             ('PAYMENT','DISCOVERY','Discover'),
                             ('PAYMENT','AUTHORIZE','Authorize'),
                             ('SESSION','FINISH_TRAN','Finish Tran')):
            b=QPushButton(label); b.setEnabled(False)
            b.clicked.connect(lambda _,g=fg,c=cmd:self.fill_template(g,c))
            fbtns.addWidget(b); self.free_cmds.append(b)
        fcbox.addLayout(fbtns)
        lay.addLayout(fcbox,2)
    ...
    def _load_mac(self):
        if os.path.exists("mac.txt"):
            ...
            if self.mac_key:
                ...
                for b in (self.status_btn,self.quick_btn,self.free_send_btn,*self.free_cmds):
                    b.setEnabled(True)
    ...
    def _sign_xml(self, raw:str)->str:
        """
        Remove any existing <MAC>, insert a placeholder immediately after
        </TRANSACTION_TIME>, compute the correct MAC, and return signed XML.
        """
        if not self.mac_key:
            raise RuntimeError("No MAC key – run EXCHANGE_KEYS first")

        # strip any MAC
        xml_no_mac=re.sub(r'<MAC>.*?</MAC>','',raw,flags=re.I|re.S)

        # find </TRANSACTION_TIME>
        m=re.search(r'</TRANSACTION_TIME\s*>',xml_no_mac,re.I)
        if not m:
            raise ValueError("Missing </TRANSACTION_TIME>")
        insert_pos=m.end()

        xml_ph=xml_no_mac[:insert_pos]+'\n  <MAC></MAC>'+xml_no_mac[insert_pos:]
        mac=calc_mac(xml_ph,self.mac_key)
        return xml_ph.replace('<MAC></MAC>',f'<MAC>{mac}</MAC>',1)

    # ───────── template builder ─────────────────────────────────────────
    def _template_xml(self, fg:str, cmd:str)->str:
        if not self.session: self.session=rand_session()
        ts=datetime.now().strftime("%m.%d.%Y %H:%M:%S")
        xml=(f"<TRANSACTION><FUNCTION_GROUP>{fg}</FUNCTION_GROUP><COMMAND>{cmd}</COMMAND>"
             f"<SESSION_ID>{self.session}</SESSION_ID><TRAINING_MODE>{int(self.training_chk.isChecked())}</TRAINING_MODE>"
             f"<TRANSACTION_TIME>{ts}</TRANSACTION_TIME>")
        if cmd=='START_TRAN':
            xml+=("<INVOICE>100000</INVOICE><CASHIER_ID>1</CASHIER_ID>"
                  "<SHIFT_ID>1</SHIFT_ID>"
                  f"<BUSINESSDATE>{datetime.now():%Y%m%d}</BUSINESSDATE>")
        elif cmd=='DISCOVERY':
            xml+=("<TIMEOUT>60</TIMEOUT><TRANSACTION_DETAILS>"
                  "<RESTRICT_TOKEN>0</RESTRICT_TOKEN><MANUAL>0</MANUAL><CTLS>1</CTLS>"
                  "<ALLOW_CANCEL>1</ALLOW_CANCEL><UNATTENDED>0</UNATTENDED>"
                  "<TRAN_TYPE>01</TRAN_TYPE><MTI>100</MTI><ENTRY_MODE>04</ENTRY_MODE>"
                  "<TRANSACTION_AMOUNT>000000001000</TRANSACTION_AMOUNT>"
                  "<ORIGINAL_CURRENCY>376</ORIGINAL_CURRENCY></TRANSACTION_DETAILS>")
        elif cmd=='AUTHORIZE':
            xml+=("<TRANSACTION_DETAILS>"
                  "<OPERATION>04</OPERATION><TRAN_TYPE>01</TRAN_TYPE><BATCH_ID>0</BATCH_ID>"
                  "<MTI>100</MTI><TRANSACTION_AMOUNT>000000001000</TRANSACTION_AMOUNT>"
                  "<ORIGINAL_CURRENCY>376</ORIGINAL_CURRENCY><CREDIT_TERMS>1</CREDIT_TERMS>"
                  "<RESTRICT_TOKEN>0</RESTRICT_TOKEN></TRANSACTION_DETAILS>")
        xml+="</TRANSACTION>"
        return self._sign_xml(xml)

    # ───────── XML envelope for internal calls ──────────────────────────
    def _env(self, fg:str, cmd:str, body:str="")->str:
        ...
        hdr=(f"<TRANSACTION><FUNCTION_GROUP>{fg}</FUNCTION_GROUP><COMMAND>{cmd}</COMMAND>"
             ...
             f"<TRANSACTION_TIME>{ts}</TRANSACTION_TIME>")
        xml=hdr+body+"</TRANSACTION>"
        return self._sign_xml(xml)

    # ───────── Free-Call slots ──────────────────────────────────────────
    def fill_template(self, fg:str, cmd:str): self.free_xml.setPlainText(self._template_xml(fg,cmd))
    def free_send(self):
        xml=self.free_xml.toPlainText().strip()
        if not xml:
        ...
        self.free_xml.setPlainText(signed)
        self._send(signed)
    ...
    def _handle(self, sent:str, recv:str):
        ...
        if '<COMMAND>CANCEL' in sent:
            code=re.search(r'<RESULT_CODE>(\d+)<',recv)
            ok=code and code.group(1) in ('0','50','51')
            if not ok:
                QMessageBox.warning(self,"Cancel failed",f"code {code.group(1) if code else '?'}")
        ...
        if '<COMMAND>EXCHANGE_KEYS' in sent and '<MAC_KEY>' in recv:
            if (m:=re.search(r'<MAC_KEY>([^<]+)</MAC_KEY>',recv)):
                try:
                    ...
                    for b in (self.status_btn,self.quick_btn,self.free_send_btn,*self.free_cmds): b.setEnabled(True)
