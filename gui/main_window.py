...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Read webhook_host/webhook_port (defaults to localhost:8080)
        ...
        # Start local HTTP server to handle /pay?amount=... callbacks
    ...
    def _load_mac(self):
        """
        After a successful EXCHANGE_KEYS, enable all related buttons.
        """
        key = load_mac()
        if key:
            self.mac_key = key
    ...
    def _sign_xml(self, raw: str) -> str:
        """
        Insert and compute <MAC> tag in raw XML.
        Raises if no MAC key is loaded.
        """
        if not self.mac_key:
    ...
    def fill_template(self, fg: str, cmd: str):
        """
        Populate sandbox editor with a signed template for (fg,cmd).
        """
        xml = template_xml(
            fg=fg,
            cmd=cmd,
    ...
    def free_send(self):
        """
        Read raw XML, sign it, replace editor content, and send to device.
        """
    ...
    def _send(self, xml: str):
        """
        Spawn a SockThread to send `xml` to device at (ip_field,port_field).
        """
        try:
    ...
    def quick_sale(self, amount: float, tcode: str='01'):
        """
        Orchestrate a full Quick-Sale:
        START_TRAN → DISCOVERY → AUTHORIZE → SAVE_RECEIPT → FINISH_TRAN.
        """
        if self.qs_active:
        ...
        # START_TRAN body (invoice + POS_TYPE)
        self.qs_start_body = (
    ...
    def _from_webhook(self, amount: float, tcode: str):
        """
        Triggered by WebhookServer when /pay?amount=... arrives.
        Starts a quick_sale with those params.
        """
    ...
    def search_logs(self):
        """
        Highlight occurrences of search term in both logs.
        """
    ...
    # ───── Handling Responses & FSM ─────
    def _schedule_cancel(self):
        """
        Schedule a GENERAL CANCEL after 2 seconds on RESULT_CODE 2.
        """
    ...
    def _send_cancel(self):
        """
        Send GENERAL CANCEL command for auto-cancel logic.
        """
    ...
    def _handle_response(self, sent: str, recv: str):
        """
        Central handler for all device responses:
        - Append to logs
        - Auto-cancel on RESULT_CODE 2
        - Drive Quick-Sale FSM
        - Handle CANCEL responses
        - Enable KTK entry after REGISTER
        - Decrypt & store MAC key after EXCHANGE_KEYS
        """
        # Append to UI logs
        ...
        if self.qs_active:
            ...
            if '<COMMAND>DISCOVERY' in sent:
                ...
                # Build flags and min/max payments
                flags = {
        ...
        if '<COMMAND>CANCEL' in sent:
            ...
            self.qs_active       = False
            # Ensure transaction ends cleanly
            xml = env_xml(
                fg="SESSION", cmd="FINISH_TRAN",
                session=self.session,
                training=bool(int(self.train_combo.currentText())),
        ...
        # After EXCHANGE_KEYS returns <MAC_KEY>, decrypt & persist
    ...
    def _disc_body(self, opts: dict) -> str:
        """
        Return the inner XML for DISCOVERY using opts dict.
        """
        x = (
    ...
    def _auth_body(self, base: dict, ct: str, pay: int, first: float, nxt: float) -> str:
        """
        Extend DISCOVERY body with credit/installment fields.
        """
