...
# 2025-05-08  v1.36
#
# • Auto-recovery: 2 s after any RESULT_CODE 2 the app sends GENERAL/CANCEL.
# • CANCEL answers 0 = OK, 50/51 = “already done” are all treated as success
#   and followed by SESSION/FINISH_TRAN to release the terminal.
# • When a Quick-Sale is started the script creates a fresh 16-digit
#   SESSION_ID if none exists, so subsequent CANCEL carries a real ID.
...
class SockThread(QThread):
    result = pyqtSignal(str, str)               # xml-sent, xml-received
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                try:
                    ...
                    return self._reply(400, b"amount must be a positive number")
                ...
                outer.receivedPayment.emit(amt, code)
...
class CreditTermDlg(QDialog):
    ...
    def __init__(self, flags: Dict[str, bool],
                 mn: int, mx: int, total: float, parent=None):
        super().__init__(parent)
        self.total = total
        self.setWindowTitle("Credit / Installment")
        g = QGridLayout(self); row = 0

        g.addWidget(QLabel("Supported terms:"), row, 0, 1, 2); row += 1
        self.cmb = QComboBox()
        for lbl, code in self._map.items():
            ...
            self.cmb.model().item(self.cmb.count()-1)\
                .setEnabled(flags.get(field, False))
        g.addWidget(self.cmb, row, 0, 1, 2); row += 1

        self.pay_lbl, self.pay_spin = QLabel("Payments:"), QSpinBox(
            minimum=mn, maximum=mx, value=mn)
        g.addWidget(self.pay_lbl, row, 0)
        g.addWidget(self.pay_spin, row, 1); row += 1

        self.first_lbl, self.first_spin = QLabel("First ₪:"), QDoubleSpinBox(
            decimals=2, maximum=total, value=total/mn)
        self.next_lbl, self.next_spin  = QLabel("Next ₪:"), QDoubleSpinBox(
            decimals=2, maximum=total, value=self.first_spin.value())
        self.next_spin.setReadOnly(True)
        g.addWidget(self.first_lbl, row, 0)
        g.addWidget(self.first_spin, row, 1); row += 1
        g.addWidget(self.next_lbl, row, 0)
        g.addWidget(self.next_spin, row, 1); row += 1
        ...
        g.addWidget(box, row, 0, 1, 2)

    def _toggle(self, *_):
        vis = self.cmb.currentData() in ('6','8')
        for w in (self.pay_lbl, self.pay_spin, self.first_lbl,
                  self.first_spin, self.next_lbl, self.next_spin):
            w.setVisible(vis)
        self._recalc()
    ...
    def _recalc(self, *_):
        if not self.first_spin.isVisible():
            return
        nxt = max(0.0, self.total - self.first_spin.value()) \
              / max(1, self.pay_spin.value()-1)
    ...
    def data(self)->Tuple[str,int,float,float]:
        ...
        if c in ('6','8'):
            return c, self.pay_spin.value(), \
                   self.first_spin.value(), self.next_spin.value()
        return c, 0, 0.0, 0.0
...
class DiscoverDlg(QDialog):
    def __init__(self, parent=None):
        ...
        g.addWidget(QLabel("Timeout (s)"), row, 0)
        g.addWidget(self.timeout, row, 1); row += 1

        self.manual = QCheckBox("Manual entry")
        g.addWidget(self.manual, row, 0)
        self.man_reason = QComboBox()
        self.man_reason.addItems(["SIG", "CNP"])
        self.man_reason.setEnabled(False)
        g.addWidget(self.man_reason, row, 1); row += 1
        ...
        g.addWidget(QLabel("Tran. type"), row, 0)
        g.addWidget(self.tran_type, row, 1); row += 1

        self.amount = QDoubleSpinBox(decimals=2, maximum=999999, value=10.00)
        g.addWidget(QLabel("Bill ₪"), row, 0)
        g.addWidget(self.amount, row, 1)
        self.currency = QComboBox()
        self.currency.addItems(["376 NIS","840 USD","978 EUR"])
        g.addWidget(QLabel("Curr"), row, 2)
        g.addWidget(self.currency, row, 3); row += 1

        self.cash_chk = QCheckBox("Cash?")
        self.cash_amt = QDoubleSpinBox(decimals=2, maximum=999999)
        self.cash_amt.setEnabled(False)
        g.addWidget(self.cash_chk, row, 0)
        g.addWidget(self.cash_amt, row, 1); row += 1
        ...
        g.addWidget(self.fx_chk, row, 0)
        g.addWidget(QLabel("to"), row, 2)
        g.addWidget(self.fx_to, row, 3); row += 1
        g.addWidget(QLabel("Conv amt"), row, 2)
        g.addWidget(self.fx_amt, row, 3); row += 1
        self.fx_chk.toggled.connect(lambda b:[w.setEnabled(b)
                                              for w in (self.fx_to,self.fx_amt)])

        self.gen_token = QComboBox(); self.gen_token.addItems(
            ["0 None","1 All cards","2 Shufersal"])
        g.addWidget(QLabel("Gen token"), row, 0)
        g.addWidget(self.gen_token, row, 1); row += 1

        self.use_tok = QCheckBox("Use token")
        self.tok_val = QLineEdit(); self.tok_val.setEnabled(False)
        g.addWidget(self.use_tok, row, 0)
        g.addWidget(self.tok_val, row, 1); row += 1
        self.use_tok.toggled.connect(self.tok_val.setEnabled)

        self.service = QComboBox(); self.service.addItems(["","1","2","3"])
        g.addWidget(QLabel("Service type"), row, 0)
        g.addWidget(self.service, row, 1); row += 1
        ...
        g.addWidget(self.ctls,row,0)
        g.addWidget(self.allow,row,1)
        g.addWidget(self.unatt,row,2); row += 1

        self.op = QComboBox(); self.op.addItems(
            ["03 Inquiry","04 Execute","05 Authorize","06 Capture"])
        g.addWidget(QLabel("Operation"), row, 0)
        g.addWidget(self.op, row, 1); row += 1

        box = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        box.button(QDialogButtonBox.Ok).setText("Discover")
        box.accepted.connect(self.accept)
        box.rejected.connect(self.reject)
        g.addWidget(box, row, 0, 1, 4)
    ...
    def data(self)->Dict[str,Optional[str]]:
        return {
            ...
            'manual_reason': self.man_reason.currentText()
                             if self.manual.isChecked() else '',
            'tran_type': self.tran_type.currentText().split()[0],
            'amount': to_minor(self.amount.value()),
            'currency': self.currency.currentText().split()[0],
            'cash': to_minor(self.cash_amt.value())
                    if self.cash_chk.isChecked() else None,
            'fx': self.fx_chk.isChecked(),
            'fx_to': self.fx_to.currentText().split()[0]
                     if self.fx_chk.isChecked() else '',
            'fx_amt': to_minor(self.fx_amt.value())
                      if self.fx_chk.isChecked() else None,
...
class Main(QMainWindow):
    def __init__(self):
        ...
        mac_r.addWidget(QLabel("MAC Key")); mac_r.addWidget(self.mac_view)
        lay.addLayout(mac_r)

        br = QHBoxLayout()
        self.start_btn = QPushButton("Start Tran…", clicked=self.do_start)
        self.start_btn.setEnabled(False)
        self.disc_btn  = QPushButton("Discover…", clicked=self.do_discover)
        ...
        self.quick_btn = QPushButton("Quick Sale",
            clicked=lambda: self.quick_sale(self.qs_amt.value(), '01'))
        self.quick_btn.setEnabled(False); br.addWidget(self.quick_btn)

        br.addWidget(QLabel("Admin"))
        ...
        self.status_btn.setEnabled(False); lay.addWidget(self.status_btn)

        self.sent = QTextEdit(readOnly=True)
        self.recv = QTextEdit(readOnly=True)
        split = QSplitter(Qt.Horizontal)
        split.addWidget(self.sent); split.addWidget(self.recv)
        lay.addWidget(split)
        ...
        srow.addWidget(self.search_edit); srow.addWidget(self.search_btn)
        lay.addLayout(srow)
    ...
    def _send(self, xml:str):
        try:
            ...
            return QMessageBox.critical(self, "Port", "Invalid port")
    ...
    def _auth_body(self, base:Dict[str,str],
                   ct:str, pay:int, first:float, nxt:float)->str:
        ...
        return x.replace('</TRANSACTION_DETAILS>',
                         inj + '</TRANSACTION_DETAILS>')
    ...
    def _schedule_cancel(self):
        ...
        self._cancel_timer.start(2000)   # 2 s
    ...
    def cmd_keys(self):
        ...
        self._send(self._env("ADMIN","EXCHANGE_KEYS",
                             f"<KTK>{self.ktk}</KTK>"))
    ...
    def do_discover(self):
        ...
        self._send(self._env("PAYMENT","DISCOVERY",
                             self._disc_body(dlg.data())))

    def quick_sale(self, amount:float, tcode:str='01'):
        if self.qs_active:
            return QMessageBox.warning(self,"Quick Sale",
                                       "Already running.")
        if not self.session:                          # ensure session
            self.session = rand_session()
        ...
        self.qs_defaults = {
            ...
            'service_type':'',
            'ctls':True, 'allow_cancel':True,
            'unattended':False,
            'operation':'04'
        }
        self.qs_start_body = ("<INVOICE>100000</INVOICE>"
                              f"<POS_TYPE>{self.pos_type.currentText()}</POS_TYPE>")
        self.qs_active = True
        self._send(self._env("SESSION","START_TRAN",
                             self.qs_start_body))
    ...
    def _handle(self, sent:str, recv:str):
        ...
        # 1. global trap: RESULT_CODE 2 → schedule CANCEL
        ...
        if self.qs_active:
            ...
            if '<COMMAND>DISCOVERY' in sent:
                ...
                f = lambda n: bool(re.search(
                    fr'<TERMS_{n.upper()}>1</TERMS_{n.upper()}>', recv, re.I))
                flags = {k:f(k) for k in ('regular','special','immediate',
                                           'credit','installments')}
                credit_ok = flags['credit'] or flags['installments']
                if credit_ok:
                    it = lambda tag,d: int(re.search(fr'<{tag}>(\d+)',recv).group(1)) \
                        if re.search(fr'<{tag}>\d+',recv) else d
                    dlg = CreditTermDlg(flags,
                                        max(2,it('CREDIT_MIN_PAYMENTS',2)),
                                        max(2,it('CREDIT_MAX_PAYMENTS',36)),
                                        self.qs_amt.value(), self)
                    if dlg.exec_() != QDialog.Accepted:
                        self.qs_active = False
                        QMessageBox.information(self,"Quick Sale","Cancelled")
                        return
                    ct,pay,first,nxt = dlg.data()
                else:
                    ct,pay,first,nxt = '1',0,0.0,0.0

                self._send(self._env("PAYMENT","AUTHORIZE",
                                     self._auth_body(self.qs_defaults,
                                                     ct,pay,first,nxt)))
                return

            if '<COMMAND>AUTHORIZE' in sent:
                if '<RESULT_CODE>0<' in recv:
                    if m:=re.search(r'<RECEIPT_ARR>(.*?)</RECEIPT_ARR>',
                                    recv, re.S):
        ...
        if '<COMMAND>CANCEL' in sent:
            ...
            if code in ('0','50','51'):
                # success or benign “already done”
                self._awaiting_cancel = False
                self.qs_active = False
            else:
                QMessageBox.warning(self,"Cancel failed",
                                    f"CANCEL result code {code}")
            self._send(self._env("SESSION","FINISH_TRAN"))
            return

        # 4. registration & key exchange
