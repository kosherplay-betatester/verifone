#!/usr/bin/env python3
# Verifone P400 connector – Quick‑Sale flow (selectable amount)
# 2025‑05‑07  v1.31‑A
...
def calc_mac(xml_wo: str, mac_key: str) -> str:
    digest = SHA256.new((xml_wo + mac_key).encode()).digest()
...
class Main(QMainWindow):
    def __init__(self):
        ...
        # Quick‑Sale amount
        amt_row = QHBoxLayout()
        amt_row.addWidget(QLabel("Quick‑Sale Amount (₪)"))
        self.qs_amt = QDoubleSpinBox(decimals=2, maximum=999999, value=10.00)
        amt_row.addWidget(self.qs_amt); amt_row.addStretch()
        ...
        # KTK / key exchange row
        key_row = QHBoxLayout()
        key_row.addWidget(QLabel("KTK (16)"))
        self.ktk_edit = QLineEdit(); self.ktk_edit.setEnabled(False); key_row.addWidget(self.ktk_edit)
        self.key_btn = QPushButton("Exchange Keys", clicked=self.cmd_keys); self.key_btn.setEnabled(False)
        key_row.addWidget(self.key_btn); key_row.addStretch()
    ...
    def search_logs(self):
        ...
        for pane in (self.sent, self.recv):
            self._hilite(pane, term)

    def _hilite(self, pane: QTextEdit, word: str):
