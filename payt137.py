#!/usr/bin/env python3
# Verifone P400 connector – Quick-Sale flow + webhook trigger + receipt writer
# 2025-05-08  v1.37
#
# – Automatic 2-second GENERAL/CANCEL after any RESULT_CODE 2
# – CANCEL codes 0, 50, 51 are all deemed success and followed by FINISH_TRAN
# – On successful PAYMENT/AUTHORIZE (RESULT_CODE 0) a human-readable TXT receipt
#   is saved under ./receipts/YYMMDD_HHMMSS_receipt.txt
# – If no SESSION_ID exists when Quick-Sale starts, a fresh one is generated
...
def rand_session() -> str:                   # 16-digit session id
    return ''.join(random.choice('0123456789') for _ in range(16))

def to_minor(val: float) -> str:             # ₪→agorot
    return str(int(round(val*100)))

def calc_mac(xml: str, key: str) -> str:     # simple SHA-256 MAC (test only)
    ...
    def run(self):
        ...
        class H(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                if p.path!='/pay': return self._reply(404,b"Not Found")
                q=parse_qs(p.query)
                if 'amount' not in q: return self._reply(400,b"amount param required")
                try:
                    amt=float(q['amount'][0]); assert amt>0
                except Exception:
                    return self._reply(400,b"amount must be positive number")
                code=q.get('type',['01'])[0]
                if code not in VALID_TYPES: return self._reply(400,b"invalid type code")
...
class DiscoverDlg(QDialog):
    def __init__(self,parent=None):
        ...
        self.fx_chk=QCheckBox("Convert FX"); self.fx_to=QComboBox(); self.fx_to.addItems(["376 NIS","840 USD","978 EUR"])
        self.fx_to.setEnabled(False); self.fx_amt=QDoubleSpinBox(decimals=2,maximum=999999); self.fx_amt.setEnabled(False)
        ...
        self.ctls=QCheckBox("CTLS"); self.ctls.setChecked(True); self.allow=QCheckBox("Allow cancel"); self.allow.setChecked(True)
...
class Main(QMainWindow):
    ...
    def _save_receipt(self, xml:str):
        ...
        curr=tag('ORIGINAL_CURRENCY') or ''
        result=tag('RESULT_TEXT') or ''
        lines=[ "Verifone P400 – Receipt",
                "========================",
                f"Date/Time      : {ts}",
                f"Amount         : {amount:.2f} {curr}",
                f"Result         : {result}",
                "",
                "Card",
                "----",
                f"Masked PAN     : {tag('MASKED_PAN') or 'N/A'}",
                f"Brand / Issuer : {tag('BRAND') or 'N/A'} / {tag('ISSUER') or 'N/A'}",
                f"Expiry         : {tag('CARD_EXPIRY') or 'N/A'}",
                "",
                "Transaction",
                "-----------",
                f"Type           : {tag('TRAN_TYPE') or ''}",
                f"Auth No.       : {tag('ISSUER_AUTH_NUM') or tag('AQUIRER_AUTH_NUM') or 'N/A'}",
                f"Trans-ID       : {tag('TRANS_ID') or 'N/A'}",
                f"Ref-No (RRN)   : {tag('RRN') or 'N/A'}",
                "" ]
        ...
        QMessageBox.information(self,"Receipt saved",f"Receipt written to:\n{fname}")

    # ───────── auto-cancel helpers ─────────
    def _schedule_cancel(self):
        if self._awaiting_cancel: return
        self._awaiting_cancel=True; self._cancel_timer=QTimer(self)
    ...
    def _handle(self, sent:str, recv:str):
            ...
            if '<COMMAND>DISCOVERY' in sent:
                ...
                if credit_ok:
                    ...
                    if dlg.exec_()!=QDialog.Accepted:
                        self.qs_active=False; QMessageBox.information(self,"Quick Sale","Cancelled"); return
        ...
        if '<COMMAND>CANCEL' in sent:
            ...
            if not ok: QMessageBox.warning(self,"Cancel failed",
                                           f"CANCEL result code {code.group(1) if code else '?'}")
