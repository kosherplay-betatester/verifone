#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# gui/main_window.py  –  FULL FILE  (v1.57 • Ping + Status before Quick-Sale)
...
• Manual Register / Exchange-Keys / Status / generic ADMIN commands
...
class MainWindow(QMainWindow):
    ...
    # ───────────────────────────────────────────────────────
    # Message helpers – כתיבה ללוג-פאנלים בלבד
    # ───────────────────────────────────────────────────────
    ...
    def __init__(self):
        ...
        self._qs_phase             = ""    # מעקב שלבי Ping/Status/...
        ...
        try:    port = int(settings.get("webhook_port", 8080))
    ...
    # ---------- launch Quick-Sale: Ping → Status → Start Tran ----------
    def _launch_qs(self, amount: float, tcode: str):
        self.session         = rand_session()       # always fresh session
        ...
        self.qs_active = True
        self._qs_phase = "PING"
        ping_xml = env_xml("ADMIN", "PING", self.session,
                           bool(int(self.train_combo.currentText())), self.mac_key)
        self._send(ping_xml)

    # helpers for phased flow
    ...
    # ══════════════════════════════════════════════════════
    # Central response handler – כולל FSM מורחב
    # ══════════════════════════════════════════════════════
