...
class CreditTermDlg(QDialog):
    """
    Dialog shown after DISCOVERY if credit/installment is supported.
    Allows the user to pick number of payments, first payment amount, etc.
    """
    ...
    def __init__(self, flags: Dict[str,bool], mn: int, mx: int, total: float, parent=None):
        """
        flags: which credit terms are supported (keys: regular, special, immediate, credit, installments)
        mn, mx: min/max installment counts
        total: total amount to be financed
        """
        ...
        for label, code in self._map.items():
            ...
            # enable only if supported by flags
    ...
    def _toggle(self):
        """
        Show/hide installment controls only if 'credit' or 'installments' selected.
        """
    ...
    def _recalc(self):
        """
        Recalculate the 'next payment' amount based on total - first_payment.
        """
    ...
    def data(self) -> Tuple[str,int,float,float]:
        """
        Return a tuple (credit_term_code, num_payments, first_amount, next_amount),
        or ('code',0,0.0,0.0) if no installment selected.
        """
...
class DiscoverDlg(QDialog):
    """
    Dialog for DISCOVERY command:
    choose timeout, manual entry, transaction type, FX options, token usage, etc.
    """
    ...
    def data(self) -> Dict:
        """
        Return a dict with all DISCOVERY parameters, converting numeric fields
        to strings as expected by templating logic.
        """
        return {
