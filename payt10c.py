#!/usr/bin/env python3
# Verifone P400 connector – full Quick‑Sale flow
# 2025‑05‑06
...
def des3_decrypt(ktk: str, mac_b64: str) -> str:
    """Decrypt MAC_KEY from EXCHANGE_KEYS response."""
    if len(ktk) != 16:
...
def calc_mac(xml_without_mac: str, mac_key: str) -> str:
    digest = SHA256.new((xml_without_mac + mac_key).encode()).digest()
    return b64encode(digest).decode()


def to_minor(val: float) -> str:
    """Convert 10.00 ₪ → 1000 agorot for XML."""
    return str(int(round(val * 100)))


# ────────────────────── threaded TCP sender ──────────────────────
class SockThread(QThread):
    result = pyqtSignal(str, str)              # sent_xml, received_xml
...
class DiscoverDlg(QDialog):
    """Full Discover dialog identical to previous build."""
    def __init__(self, parent=None):
        super().__init__(parent)
...
class Main(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Verifone P400 Connector – Quick Sale 4‑step")
        ...
        # quick‑sale queue: list[(fg, cmd, body)]
        ...
        for b in (self.start_btn, self.disc_btn, self.quick_btn): btns.addWidget(b)
        btns.addWidget(QLabel("Admin cmd"))
        self.cmd_in = QLineEdit()
        self.cmd_send = QPushButton("Send", clicked=self.admin_cmd)
        btns.addWidget(self.cmd_in); btns.addWidget(self.cmd_send); btns.addStretch(); lay.addLayout(btns)
    ...
    # ------------------- DISCOVERY & AUTHORIZE helpers --------------------
    def _disc_body(self, d: Dict[str, Optional[str]], add_credit_terms=False) -> str:
        trans = (f"<TRANSACTION_DETAILS>"
        ...
        if d['manual'] and d['manual_reason']:
            trans += f"<MANUAL_REASON>{d['manual_reason']}</MANUAL_REASON>"
        if d['cash'] is not None:
            trans += f"<CASH_AMOUNT>{d['cash']}</CASH_AMOUNT>"
        if d['fx']:
            trans += (f"<CONVERTED_AMOUNT>{d['fx_amt']}</CONVERTED_AMOUNT>"
                      f"<CONVERTED_CURRENCY>{d['fx_to']}</CONVERTED_CURRENCY>")
        if d['use_token'] and d['token_val']:
            trans += f"<CARD_TOKEN>{d['token_val']}</CARD_TOKEN>"
        if d['service_type']:
            trans += f"<SERVICE_TYPE>{d['service_type']}</SERVICE_TYPE>"
        if add_credit_terms:
            trans += "<CREDIT_TERMS>3</CREDIT_TERMS>"
        trans += f"<OPERATION>{d['operation']}</OPERATION></TRANSACTION_DETAILS>"
        return f"<TIMEOUT>{d['timeout']}</TIMEOUT>" + trans
    ...
    def quick_sale(self):
        ...
        disc_body = self._disc_body(disc_defaults)

        # 3) AUTHORIZE with CREDIT_TERMS=3
        auth_body = self._disc_body(disc_defaults, add_credit_terms=True)

        # 4) FINISH_TRAN has no body
        self.qs_queue = [
    ...
    def _handle(self, sent_xml: str, recv_xml: str):
        ...
        # Quick‑Sale sequencing
        if self.qs_queue:
            # proceed when RESULT_CODE 0 or EVENT COMPLETED is seen
            ok = '<RESULT_CODE>0<' in recv_xml or '<EVENT>COMPLETED' in recv_xml
            if ok:
                # If Authorize succeeded, show receipt
                if '<COMMAND>AUTHORIZE' in sent_xml and '<RESULT_CODE>0<' in recv_xml:
                    if m := re.search(r'<RECEIPT_ARR>(.*?)</RECEIPT_ARR>', recv_xml, re.S):
                        receipt = html.unescape(m.group(1).strip())
                        QMessageBox.information(self, "Receipt (UTF‑8)", receipt)
                if self.qs_queue:
                    fg, cmd, body = self.qs_queue.pop(0)
                    self._send(self._env(fg, cmd, body))
            else:
                # Abort queue if any failure
                self.qs_queue.clear()

        # Register/key UI state
