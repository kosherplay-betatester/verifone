...
# 2025-05-13  v1.43  (Improved KTK error handling)
#
...
# • User-friendly error message for incorrect KTK during Exchange Keys.
...
def des3_decrypt(ktk: str, mac_b64: str) -> Optional[str]:
    if len(ktk) != 16:
        # This is a specific check we can keep separate for clarity if needed
        # but the main issue is the decryption result, not KTK length for this error.
        # For now, let the crypto library handle potential key length issues.
        pass
    try:
        key24 = DES3.adjust_key_parity(ktk.encode('ascii') + ktk.encode('ascii')[:8]) # Assuming KTK is ASCII
        decrypted_bytes = DES3.new(key24, DES3.MODE_ECB).decrypt(b64decode(mac_b64))
        return decrypted_bytes.rstrip(b'\0').decode('utf-8')
    except UnicodeDecodeError:
        return None # Signal that decryption produced non-UTF8 data (likely wrong KTK)
    except ValueError as ve: # Catches issues from b64decode or DES3 keying if KTK is bad format
        print(f"ValueError during decryption (possibly bad KTK format or MAC_KEY): {ve}")
        return None
    except Exception as e: # Catch other potential crypto errors
        print(f"Unexpected decryption error: {e}") # Log for debugging
        return None
...
class CreditTermDlg(QDialog):
    ...
    def _recalc(self,*_):
        ...
        remaining_payments = max(1, num_payments - 1) # Avoid division by zero if num_payments is 1
...
class Main(QMainWindow):
    ...
    def _save_receipt(self, xml:str):
        ...
        amount_str = tag('TRANSACTION_AMOUNT')
        amount = int(amount_str if amount_str else 0)/100

        curr_code = tag('ORIGINAL_CURRENCY')
        curr_map = {'376': '₪', '840': '$', '978': '€'} # Map currency codes to symbols
        curr = curr_map.get(curr_code, curr_code or '') # Use symbol or code, fallback to empty
    ...
    def cmd_register(self):
        self._set_transaction_buttons_enabled(False) # Disable while registering
        self.key_btn.setEnabled(False) # Disable key exchange until register response
        self.ktk_edit.setEnabled(False)
    ...
    def cmd_keys(self):
        ...
        if len(self.ktk) != 16:
            ...
            return
        self._set_transaction_buttons_enabled(False) # Disable while exchanging
    ...
    def admin_cmd(self):
        ...
        if cmd:
            if not self.session and cmd not in ("REGISTER", "EXCHANGE_KEYS", "STATUS", "PING"):
                self.session = rand_session()
            # For commands like EOD, OPEN_LANE, CLOSE_LANE, they might implicitly end a session
            # or start a new state. Consider disabling transaction buttons if these are sent.
            if cmd in ("EOD", "CLOSE_LANE", "OPEN_LANE", "ERASE_SHVA"):
                 self._set_transaction_buttons_enabled(False) # Disable until response clarifies state
    ...
    def _handle(self, sent:str, recv:str):
        ...
        is_cancel_sent = '<COMMAND>CANCEL' in sent
        is_finish_tran_sent = '<COMMAND>FINISH_TRAN' in sent

        if '<RESULT_CODE>2<' in recv and not is_cancel_sent and not self._awaiting_cancel:
            # Only schedule cancel for ongoing interactive transactions
            if self.qs_active or \
               ('<COMMAND>START_TRAN' in sent and not is_finish_tran_sent) or \
               ('<COMMAND>DISCOVERY' in sent and not is_finish_tran_sent) or \
               ('<COMMAND>AUTHORIZE' in sent and not is_finish_tran_sent) :
                self._schedule_cancel()
            return # Wait for CANCEL flow to complete
        ...
        if self.qs_active:
            if '<COMMAND>START_TRAN' in sent:
                if '<RESULT_CODE>0<' in recv:
                    ...
                    self.qs_active=False; self._set_transaction_buttons_enabled(True)
                    self._send(self._env("SESSION", "FINISH_TRAN")) # Attempt to clean up
                return
            if '<COMMAND>DISCOVERY' in sent:
                ...
                min_pays = int(getattr(re.search(r'<MIN_PAYMENTS>(\d+)</MIN_PAYMENTS>', recv), 'group', lambda x: '2')(1))
                max_pays = int(getattr(re.search(r'<MAX_PAYMENTS>(\d+)</MAX_PAYMENTS>', recv), 'group', lambda x: '36')(1))
                dlg=CreditTermDlg(flags,min_pays, max_pays, self.qs_amt.value(),self)

                if dlg.exec_()!=QDialog.Accepted:
                    self.qs_active=False; self._set_transaction_buttons_enabled(True)
                    QMessageBox.information(self,"Quick Sale","בוטל")
                    self._send(self._env("GENERAL", "CANCEL")) # This will trigger FINISH_TRAN
                    return
            ...
            if '<COMMAND>AUTHORIZE' in sent:
                ...
                self._send(self._env("SESSION","FINISH_TRAN")) # FINISH_TRAN will handle qs_active and buttons
                return

        # Handle flows NOT part of Quick Sale that might need button re-enabling
        if not self.qs_active:
            if ('<COMMAND>START_TRAN' in sent or '<COMMAND>DISCOVERY' in sent) and '<RESULT_CODE>0<' not in recv:
                self._set_transaction_buttons_enabled(True)
                if self.session: self._send(self._env("SESSION", "FINISH_TRAN")) # Clean up potential session
            # For admin commands that might affect session state
            if '<FUNCTION_GROUP>ADMIN</FUNCTION_GROUP>' in sent and \
               any(cmd in sent for cmd in ["EOD", "CLOSE_LANE", "OPEN_LANE", "ERASE_SHVA"]):
                if '<RESULT_CODE>0<' in recv: # Assuming 0 means success for these admin ops
                    QMessageBox.information(self, "Admin Command", f"{sent.split('<COMMAND>')[1].split('</COMMAND>')[0]} successful.")
                else:
                    QMessageBox.warning(self, "Admin Command", f"{sent.split('<COMMAND>')[1].split('</COMMAND>')[0]} failed or did not return success.")
                self._set_transaction_buttons_enabled(True) # Re-enable after admin op
                self.session = '' # Admin ops often invalidate current session

        if is_finish_tran_sent:
            self.qs_active = False
            self._set_transaction_buttons_enabled(True)
            if self._awaiting_cancel : self._awaiting_cancel = False
            self.session = ''
            return
        ...
        if is_cancel_sent:
            ...
            ok = code in ('0','50','51')
            if not ok: QMessageBox.warning(self,"Cancel failed",f"Result code: {code}")
            # FINISH_TRAN is now mandatory after CANCEL to reset state
        ...
        if '<COMMAND>REGISTER' in sent and '<EVENT>COMPLETED' in recv:
            if '<RESULT_CODE>0<' in recv:
                ...
                QMessageBox.information(self, "Register", "Register POS successful. Please enter KTK displayed on terminal.")
            else:
                QMessageBox.critical(self, "Register Failed", f"Could not register POS. Response:\n{html.escape(recv)}")
                self.session = ''
                self._set_transaction_buttons_enabled(False) # Keep disabled if register fails
                self.key_btn.setEnabled(False)
                self.ktk_edit.setEnabled(False)


        if '<COMMAND>EXCHANGE_KEYS' in sent:
            # Re-enable transaction buttons regardless of EXCHANGE_KEYS outcome here,
            # but they will be disabled again if MAC key is not set/valid.
            # This allows user to retry KTK entry without restarting.
            # self._set_transaction_buttons_enabled(True) # This was too early

            if '<RESULT_CODE>0<' in recv and (m:=re.search(r'<MAC_KEY>([^<]+)</MAC_KEY>',recv)):
                decrypted_mac_key = des3_decrypt(self.ktk, m.group(1))
                if decrypted_mac_key is not None:
                    self.mac_key=decrypted_mac_key; self.mac_view.setText(self.mac_key); self._save_mac()
                    self._set_transaction_buttons_enabled(True)
                    self.status_btn.setEnabled(True)
                    QMessageBox.information(self, "Exchange Keys", "MAC Key exchanged and saved successfully.")
                else:
                    QMessageBox.critical(self, "Decrypt Failed", "Invalid KTK entered or decryption failed. Please verify KTK and try 'Exchange Keys' again.")
                    self.mac_key = ""; self.mac_view.clear(); self._save_mac()
                    self._set_transaction_buttons_enabled(False)
                    self.status_btn.setEnabled(False)
            else:
                QMessageBox.critical(self, "Exchange Keys Failed", f"Could not exchange keys. MAC_KEY not found or error.\nResponse:\n{html.escape(recv)}")
                self.mac_key = ""; self.mac_view.clear(); self._save_mac()
                self._set_transaction_buttons_enabled(False)
                self.status_btn.setEnabled(False)
            # Ensure KTK edit and key button are re-enabled for retry if needed,
            # unless a valid MAC key was just set.
            if not self.mac_key:
                self.ktk_edit.setEnabled(True)
