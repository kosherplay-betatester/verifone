...
# 2025-05-11  v1.39  (credit-term dialog always shown)
#
# • RESULT_CODE 2  → 2-second GENERAL/CANCEL  → SESSION/FINISH_TRAN.
# • CANCEL codes 0 / 50 / 51 are considered success.
    ...
    def run(self):
        ...
        class H(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                if p.path != '/pay':
                    return self._r(404, b"Not Found")
                q = parse_qs(p.query)
                if 'amount' not in q:
                    return self._r(400, b"amount param required")
                try:
                ...
                if code not in VALID_TYPES:
                    return self._r(400, b"invalid type")
                self._r(200, b"OK")
...
class CreditTermDlg(QDialog):
    ...
    def _recalc(self, *_):
        if not self.first_spin.isVisible():          # not credit / installments
            return
    ...
    def data(self) -> Tuple[str, int, float, float]:
        ...
        if c in ('6', '8'):
            return c, self.pay_spin.value(), self.first_spin.value(), self.next_spin.value()
        return c, 0, 0.0, 0.0
...
class DiscoverDlg(QDialog):
    def __init__(self, parent=None):
        ...
        self.tran_type.addItems(["01 Regular", "02 Unloading", "03 Forced", "06 Cashback",
...
class Main(QMainWindow):
    def __init__(self):
        ...
        # persistence + webhook
        self._load_mac()
        self.webhook = WebhookServer(parent=self); self.webhook.receivedPayment.connect(self._from_webhook)
        self.webhook.start()
    ...
    def _save_mac(self):
        open("mac.txt", "w", encoding='utf-8').write(self.mac_key)
    ...
    def _handle(self, sent: str, recv: str):
        ...
        if self.qs_active:
            ...
            # PAYMENT/DISCOVERY response
            if '<COMMAND>DISCOVERY' in sent:
                if '<RESULT_CODE>0<' not in recv:          # discovery failed
                    self.qs_active = False
                    return

                # issuer flags → enable/disable items, but ALWAYS show dialog
        ...
        if '<COMMAND>CANCEL' in sent:
            ...
            if not ok:
                QMessageBox.warning(self, "Cancel failed",
                                    f"code {code.group(1) if code else '?'}")
            self._awaiting_cancel = False
            self.qs_active = False
            self._send(self._env("SESSION", "FINISH_TRAN"))
            return
