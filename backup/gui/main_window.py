#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# gui/main_window.py  –  FULL FILE  (v1.56 • no-popup errors + QDialog import)

"""
Main admin window for Verifone-P400 desktop app
==============================================

• Quick-Sale queue:  START_TRAN → DISCOVERY → AUTHORIZE → FINISH_TRAN  
• Manual Register / Exchange-Keys / Status / generic ADMIN cmds  
• Free-Call XML sandbox with auto-signing  
• Auto-cancel on RESULT_CODE 2 **and** on user “ביטול” in credit dialog  
• v1.54: AUTHORIZE reply (success *or* error) is always written to receipt.json  
• v1.55: removed all QMessageBox pop-ups — info/warn/error go to log panes  
• v1.56: **QDialog imported** (fixes NameError when checking `dlg.exec_()` result)
"""
...
from PyQt5.QtWidgets import (
    ...
    QDoubleSpinBox, QDialog                     # ← QDialog added here
)
...
class MainWindow(QMainWindow):
    ...
    # ───────────────────────────────────────────────────────
    # Message helpers  –  *log only*  (no modal pop-ups)
    # ───────────────────────────────────────────────────────
    def _mb_parent(self):
        return self if self.isVisible() else None

    def _info(self, title: str, text: str):
        self.sent_log.append(
            f'<span style="color:#27ae60;">[INFO] {title}: {text}</span>'
        )
        self.sent_log.moveCursor(QTextCursor.End)

    def _warn(self, title: str, text: str):
        self.recv_log.append(
            f'<span style="color:#e67e22;">[WARN] {title}: {text}</span>'
        )
        self.recv_log.moveCursor(QTextCursor.End)

    def _crit(self, title: str, text: str):
        self.recv_log.append(
            f'<span style="color:#c0392b;">[ERROR] {title}: {text}</span>'
        )
        self.recv_log.moveCursor(QTextCursor.End)
    ...
    def __init__(self):
        ...
        # Key-exchange row ---------------------------------------------------
        keys = QHBoxLayout()
        self.ktk_field = QLineEdit(settings.get("ktk", "")); self.ktk_field.setEnabled(False)
        ...
        self.qs_amount = QDoubleSpinBox(decimals=2, maximum=999999,
                                        value=settings.get("quick_amount", 0.0))
        act.addWidget(self.qs_amount)
        self.qs_btn = QPushButton("Quick Sale"); self.qs_btn.setEnabled(bool(self.mac_key))
        ...
        fr = QHBoxLayout()
        self.free_send_btn = QPushButton("Send Free Call"); self.free_send_btn.setEnabled(bool(self.mac_key))
        ...
        for fg, cmd, lbl in [
            ...
            fr.addWidget(b); self.free_cmds.append(b)
        sb.addLayout(fr)
    ...
    # ══════════════════════════════════════════════════════
    # Register / Keys / Status
    # ══════════════════════════════════════════════════════
    def cmd_register(self):
        self.session = rand_session()
        body = (f"<CHAIN>{self.chain_field.text()}</CHAIN>"
    ...
    def _start_next_qs(self):
        if self.qs_active or not self.qs_queue:
            return
        amount, tcode = self.qs_queue.pop(0)
        self._launch_qs(amount, tcode)
    ...
    # ══════════════════════════════════════════════════════
    # Helper: build DISCOVERY / AUTHORIZE bodies
    # ══════════════════════════════════════════════════════
    def _disc_body(self, o: dict) -> str:
        ...
        if o["manual"] and o["manual_reason"]:
            x += f"<MANUAL_REASON>{o['manual_reason']}</MANUAL_REASON>"
        if o["cash"] is not None:
            x += f"<CASH_AMOUNT>{o['cash']}</CASH_AMOUNT>"
        if o["fx"]:
            x += (f"<CONVERTED_AMOUNT>{o['fx_amt']}</CONVERTED_AMOUNT>"
                  f"<CONVERTED_CURRENCY>{o['fx_to']}</CONVERTED_CURRENCY>")
        if o["use_token"] and o["token_val"]:
            x += f"<CARD_TOKEN>{o['token_val']}</CARD_TOKEN>"
        if o["service_type"]:
            x += f"<SERVICE_TYPE>{o['service_type']}</SERVICE_TYPE>"
        x += f"<OPERATION>{o['operation']}</OPERATION></TRANSACTION_DETAILS>"
        return f"<TIMEOUT>{o['timeout']}</TIMEOUT>" + x
    ...
    def _sign_xml(self, raw: str) -> str:
        return raw if re.search(r"<MAC>.*?</MAC>", raw, re.S) else sign_xml(raw, self.mac_key)
    ...
    def _handle_response(self, sent: str, recv: str):
        ...
        # RESULT_CODE 2 → automatic cancel
        ...
        if self.qs_active:
            ...
            # DISCOVERY response → Credit dialog ---------------------------
            if "<COMMAND>DISCOVERY" in sent:
                ...
                mn = max(2, _gi("CREDIT_MIN_PAYMENTS", 2))
                mx = max(2, _gi("CREDIT_MAX_PAYMENTS", 36))

                dlg = CreditTermDlg(flags, mn, mx, self._current_amount, None)

                # User pressed “ביטול” → send CANCEL **first**
                if dlg.exec_() != QDialog.Accepted:
                    self._send_cancel()
                    return

                # Otherwise continue with AUTHORIZE
            ...
            # AUTHORIZE → save receipt, then FINISH_TRAN --------------------
        ...
        if ("<COMMAND>EXCHANGE_KEYS" in sent and "<MAC_KEY>" in recv
            ...
            if m:
                ...
                try:
                    ...
                    for b in (self.status_btn, self.qs_btn,
...
# ══════════════════════════════════════════════════════════
if __name__ == "__main__":                                # debug-launch
