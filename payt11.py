...
def des3_decrypt(ktk: str, mac_b64: str) -> str:
    """Decrypt MAC_KEY given 16‑char KTK and Base64 blob from EXCHANGE_KEYS."""
    if len(ktk) != 16:
...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.mac_key  = ''
        self.threads  = []
        self.qs_queue: List[Tuple[str, str, str]] = []        # quick‑sale steps
        ...
        macrow.addWidget(self.mac_view); macrow.addStretch()
        lay.addLayout(macrow)
        ...
        btns.addWidget(self.cmd_in); btns.addWidget(self.cmd_send); btns.addStretch()
        lay.addLayout(btns)
        ...
        # search bar
        findRow = QHBoxLayout()
        findRow.addWidget(QLabel("Search logs:"))
        ...
        findRow.addWidget(self.search_edit); findRow.addWidget(self.search_btn); findRow.addStretch()
        lay.addLayout(findRow)

        # Ctrl+F shortcut → focus search box
    ...
    # body‑builder for DISCOVERY / AUTHORIZE
    def _disc_body(self, d: Dict[str, Optional[str]], add_credit=False) -> str:
    ...
    def quick_sale(self):
        ...
        auth_body = self._disc_body(defaults, add_credit=True)
    ...
    def search_logs(self):
        ...
        for pane in (self.sent, self.recv):
            self._highlight(pane, word)

    def _highlight(self, pane: QTextEdit, word: str):
        pane.setExtraSelections([])                      # clear old
        if not word:
            return
    ...
    def _handle(self, sent_xml: str, recv_xml: str):
        ...
        if self.qs_queue:
            ...
            if ok:
                ...
                self.qs_queue.clear()                    # abort chain on error

        # enable key exchange after successful REGISTER
